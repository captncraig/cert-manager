...
func GenerateTemplate(issuer v1alpha1.GenericIssuer, crt *v1alpha1.Certificate, serialNo *big.Int, lifetimeDays int) (*x509.Certificate, error) {
	commonName := CommonNameForCertificate(crt)
	dnsNames := DNSNamesForCertificate(crt)
	if len(commonName) == 0 && len(dnsNames) == 0 {
		return nil, fmt.Errorf("no domains specified on certificate")
	...
	return &x509.Certificate{
		...
		NotBefore: time.Now(),
		NotAfter:  time.Now().Add(time.Hour * 24 * time.Duration(lifetimeDays)),
